...
fn _complete_tree_size(node: Option<&Node>) -> usize {
    if let Some(node) = node {
        return 1
            + _complete_tree_size(node.left.as_deref())
            + _complete_tree_size(node.right.as_deref());
    }
    0
}
...
fn complete_tree_size_zero() {
    ...
    let path_oram = Oram::new(Vec::new(), nb_items);
...
fn complete_tree_size_one() {
    ...
    if let Ok(path_oram) = path_oram {
        let tree_size = _complete_tree_size(path_oram.tree().root.as_deref());
...
fn access_bad_path() {
    ...
    if let Ok(mut path_oram) = path_oram {
        let path = 1337;
        let res_access = path_oram.access(AccessType::Read, path);
...
fn access_valid_path1() {
    ...
    let res_oram = Oram::new(Vec::new(), nb_items);
...
fn read_write_access() {
     ...
     * Server.
     */
    let res_oram = Oram::new(dummies, nb_items);
    ...
    let res_access = path_oram.access(AccessType::Write(ordered_elements), path);
...
fn read_write_access_no_change() {
    ...
    let mut ordered_elements =
        client.order_elements_for_writing(&path_values, path, path_oram.tree().height() as usize);
...
fn generate_zero_dummies() {
    let nb_items: usize = 0;
    let ct_size = 16;
    let client = ClientOram::new(nb_items);
...
fn change_position_bad1() {
    ...
    let secret_message: Vec<u8> = [
        66, 114, 117, 99, 101, 32, 83, 99, 104, 110, 101, 105, 101, 114, 32, 107, 101, 101, 112,
        115, 32, 99, 111, 110, 115, 116, 97, 110, 116, 32, 116, 105, 109, 101,
    ]
    .to_vec();

    let secret_message2: Vec<u8> = [
        66, 114, 117, 99, 101, 32, 83, 99, 104, 110, 101, 105, 101, 114, 32, 107, 101, 101, 112,
        115, 32, 99, 111, 110, 115, 116, 97, 110, 116, 32, 115, 105, 122, 101,
    ]
    .to_vec();
...
fn order_elements() {
    ...
    client.stash = [DataItem::new(elt1.clone()), DataItem::new(elt2.clone())].to_vec();
    ...
    let tree_height = 4;
    let ordered_buckets =
        client.order_elements_for_writing(&path_values_decrypted, path, tree_height);
...
fn general_behavior() {
    ...
    let dummies = res_dummies.unwrap();
    ...
    let mut csprng = CsRng::from_entropy();
    let mut new_values = Vec::with_capacity(path_oram.tree().height() as usize * BUCKET_SIZE + 2);
    ...
    let res_write = path_oram.access(AccessType::Write(ordered_elements), path);
     ...
     */
    let res_read = path_oram.access(AccessType::Read, path);
