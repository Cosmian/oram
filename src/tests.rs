use crate::{
    btree::{
        get_complete_tree_leaves_number, get_complete_tree_size, DataItem, Node,
    },
    client::ClientOram,
...
fn read_write_access() {
    ...
    let res_access =
        path_oram.access(AccessType::Write(ordered_elements), path);
...
fn change_position_bad1() {
    ...
    let secret_message: Vec<u8> = [
        66, 114, 117, 99, 101, 32, 83, 99, 104, 110, 101, 105, 101, 114, 32,
        107, 101, 101, 112, 115, 32, 99, 111, 110, 115, 116, 97, 110, 116, 32,
        116, 105, 109, 101,
    ]
    .to_vec();
...
fn encryption_integrity() {
    ...
    client
        .write_to_path(&mut oram, &mut read_data, Some(&mut new_values), path)
        .unwrap();
